  ...
  <div class="modal-dialog" role="document">
   ...
   <div class="form-group">
        <label class="font-weight-bold">Balance</label>
          <input type="text" name="balance" id="to_pay"class="form-control"  readonly required>
    </div>
           
    <input type="text" name="name" id="customer_name1" hidden class="form-control"  readonly required>
    <input type="text" name="contact" id="contact" hidden class="form-control"  readonly required>
...
<div class="container"> 
          ...
          <p style=" font-size:20px;"><strong>Customer Name: <?php echo $dname;?></strong>       
        <span style="float:right;">
          Connection Status:
          <?php if($conn_status==1){echo '<button style="  font-size:12px; width:50px;" class="btn btn-outline-success btn-sm">Active</button>';} 
          else{echo '<button style="  font-size:12px; width:80px;" class="btn btn btn-outline-danger btn-sm">Not Active</button>';}?>
</p>
          <p style="margin-top:-10px; font-size:20px;">Address: <?php echo $addr;?>
          <span style="float:right;">
          Date Registered: <?php echo $date_created;?>
        
        
        </p>
          <p style="margin-top:-18px; font-size:20px;">Phone Number: <?php echo $contact;?>
...
<?php
              ...
              require_once "config.php";
              $query ="SELECT  bill.id, bill.customer_id, customer.fname, customer.lname, customer.contact,
              ...
              INNER JOIN customer ON bill.customer_id=customer.id where bill.customer_id = '$did' ORDER BY `id` DESC ";
...
<?php 
  ...
  <tr>
      ...
      if($row['status']=='Paid'){
         echo '<center><button readonly class="btn btn-success btn-sm"><i class="fas fa-check-square"></i> Paid</button></center>';
         
      }
       if($row['status']=='Underpaid' and $row['balance']>0){
        echo '<center><button type="button" class="btn btn-outline-warning btn-sm addpaymentbtn"> Add
      Payment</button> <button class="btn btn-primary btn-sm" style="color:white; font-size:10px; margin-top:10px;"> Credit '.$row['balance'].'</button> </center>
      '; 
        
      }
        if($row['status']=='Underpaid' and $row['balance']<0){
          echo '<center><button type="button" class="btn btn-outline-warning btn-sm addpaymentbtn"> Add
        Payment</button> <button class="btn btn-danger btn-sm" style="color:white; font-size:10px; margin-top:10px;"> Debit '.$row['balance'].'</button> </center>
        '; 
        }
        if($row['status']=='Underpaid' and $row['balance']==0){
          echo '<center><button type="button" class="btn btn-outline-warning btn-sm addpaymentbtn"> Add
      ...
      ?></td>
          ...
          <td hidden><?php echo $row['bill_amount']-abs($row['balance']);?></td>
      ...
      <td hidden><?php echo $row['contact'];?></td>
      <?php
                    }
